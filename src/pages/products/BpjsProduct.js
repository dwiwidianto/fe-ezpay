import React from "react";
import { Navbar, CardProduct } from "../../components";
import "../../App.css";
import style from "./bpjs.module.css";
import { Container, Row, Col, Card, Button, Form } from "react-bootstrap";

export default function BPJS() {
  return (
    <>
      <Navbar />
      <CardProduct />
      <div className="products">
        <Container>
          <Row>
            <Col>
              <Card className={style.card}>
                <Card.Body>
                  <h4>
                    <b>Bayar Tagihan Air</b>
                  </h4>
                  <Form>
                    <h5 style={{ marginTop: "20px" }}>Jenis BPJS</h5>
                    {["radio"].map((type) => (
                      <div key={`inline-${type}`} className="mb-3">
                        <Form.Check inline label="Kesehatan" name="group1" type={type} id={`inline-${type}-1`} />
                        <Form.Check inline label="Ketenagakerjaan" name="group1" type={type} id={`inline-${type}-2`} />
                      </div>
                    ))}

                    <h5 style={{ marginTop: "15px" }}>Nomor BPJS</h5>
                    <Form.Control type="number" placeholder="contoh 23489274" />

                    <h5 style={{ marginTop: "15px" }}>Pembayaran Bulan</h5>
                    <Form.Select>
                      <option>Default select</option>
                      <option>Januari</option>
                      <option>Februari</option>
                      <option>Maret</option>
                      <option>April</option>
                      <option>Mei</option>
                      <option>Juni</option>
                    </Form.Select>
                    <Card className={style.cardKeterangan}>
                      <span className={style.labelKeterangan}>Keterangan</span>
                      <ol className={style.listKeterangan}>
                        <li>Wilayah : </li>
                        <li>Nomor Tagihan : </li>
                      </ol>
                    </Card>
                    <h5>Total : </h5>
                    <div className={style.Totalbayar}>
                      <h4>Rp.</h4>
                      <Button type="submit" className={style.btn}>
                        Bayar
                      </Button>
                    </div>
                  </Form>
                </Card.Body>
              </Card>
            </Col>
          </Row>
        </Container>
      </div>
    </>
  );
}
